...
export default {
  ...
  data() {
    return {
      form: {
        idSubColor: null, // Asegúrate de inicializar el ID
        nombre: '',
      },
      rolOptions: null,
      selectedRol: null,
  ...
  methods: {
    flecha(){
        this.$router.push('/SubTabla'); // Ajusta la ruta de redirección según sea necesario
      },
    ...
    async cargarDatosUsuario() {
      const idSubColor = this.$route.params.idSubColor; // Suponiendo que pasas el ID del usuario por la URL
      try {
        const response = await axios.get(`https://localhost:7061/api/SubColor/Obtener/${idSubColor}`);
    ...
    async submitForm() {
      try {
        ...
        await axios.put(`https://localhost:7061/api/SubColor/Editar`, this.form);
        
        this.$alert('Sub Color actualizado correctamente!');
        this.$router.push('SubTable');
      } catch (error) {
