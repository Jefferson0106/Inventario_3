            ...
            <form @submit.prevent="showModal">
              <div class="input-group">
                <div class="input-with-icon">
                  ...
                  <input type="text" id="nombreCompleto" name="nombreCompleto" placeholder="Nombre de Tipo de Producto" required v-model="form.nombre">
                </div>
                <div class="input-with-icon">
                  <i class="fa-solid fa-pen icon"></i>
                  <textarea id="mensaje" name="mensaje" placeholder="Descripción (Perfil)" required v-model="form.descripcion"></textarea>
                </div>
                <div class="input-with-icon">
...
export default {
  ...
  mounted() {
    ...
    axios.get("https://localhost:7061/api/Colore/Lista")
      .then(response => {
        this.colorOptions = response.data.response.map(color => ({
          id: color.idColor,
          nombre: color.nombre
        }));
      })
    ...
    axios.get("https://localhost:7061/api/SubColor/Lista")
      .then(response => {
        this.subcolorOptions = response.data.response.map(subcolor => ({
          id: subcolor.idSubColor,
          nombre: subcolor.nombre
        }));
      })
      .catch(error => {
        console.error('Error al cargar los subcolores:', error);
      });
  },
  methods: {
    flecha() {
    ...
    async submitForm() {
      try {
        ...
        const response = await axios.post("https://localhost:7061/api/TipoProducto/Guardar", this.form);
        ...
        console.error(error);
        this.$alert('Ocurrió un error al guardar el tipo de producto.');
      } finally {
        this.hideModal();
      }
    },
    mostrarAlerta() {
      this.$alert('¡El tipo de producto ha sido creado!');
    }
  }
};
</script>
