            ...
            <div class="form-container">
              <form @submit.prevent="showModal">
                <div class="input-group">
                    ...
                    <input type="text" id="nombreCompleto" name="nombreCompleto" placeholder="Nombre De Su Categoria" required v-model="form.nombre" readonly>
                    <input type="text" id="nombreCompleto" name="nombreCompleto" placeholder="Marca De Su Categoria" required v-model="form.marca" readonly>
                    <textarea id="mensaje" name="mensaje" placeholder="Descripcion (Perfil)" required v-model="form.descripcion" readonly></textarea>
...
export default {
  ...
  data() {
    return {
      form: {
        idPermiso: null, // Asegúrate de inicializar el ID
        nombre: '',
        marca: '',
        descripcion: '',
  ...
  methods: {
    ...
    async cargarDatosUsuario() {
      const idCategoria = this.$route.params.idCategoria; // Suponiendo que pasas el ID del usuario por la URL
      try {
        const response = await axios.get(`https://localhost:7061/api/Categoria/Obtener/${idCategoria}`);
        ...
        console.error('Error al cargar los datos del Su Categoria:', error);
      }
    },
    showModal() {
    ...
    async submitForm() {
      try {
        ...
        await axios.put(`https://localhost:7061/api/Categoria/Editar`, this.form);
        
        this.$alert('Categoria actualizado correctamente!');
        this.$router.push('CategoriaTable');
      } catch (error) {
        console.error('Error al actualizar Su Categoria:', error);
        this.$alert('Ocurrió un error al actualizar Su Categoria.');
      } finally {
        this.hideModal();
      }
    }
