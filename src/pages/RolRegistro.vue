  ...
  export default {
    ...
    data() {
      return {
        ...
        PermisoDal: [], // Asegúrate de inicializar como un array
        selectedRoles: [], // Cambia esto para manejar múltiples selecciones
    ...
    mounted() {
      let direccion = "https://localhost:7061/api/Permiso/Lista"; // URL para obtener la lista de permisos
      axios.get(direccion).then(response => {
        console.log(response.data.response); // Verifica la estructura de los datos
        this.PermisoDal = response.data.response.map(permiso => ({
          idPermiso: permiso.idPermiso,
          nombre: permiso.nombre // Asegúrate de que esta propiedad exista
        }));
      }).catch(error => {
    ...
    methods: {
      ...
      mostrarAlerta() {
        this.$alert('¡El Perfil ha sido creado!');
      },
      async submitForm() {
        try {
          // Primero, guarda el nuevo rol
          const roleResponse = await axios.post("https://localhost:7061/api/Role/Guardar", this.form);
          const savedRole = roleResponse.data.savedRole; // Usa el rol guardado devuelto por la API
          console.log('Rol guardado:', savedRole);
  
          // Después, guarda la relación en la tabla rolPermiso para cada permiso seleccionado
          const rolPermisoRequests = this.selectedRoles.map(rol => ({
            idRol: savedRole.idRol, // Usa el ID del rol guardado
            idPermiso: rol.idPermiso
          }));
  
          // Enviar todas las solicitudes en paralelo
          await Promise.all(rolPermisoRequests.map(data =>
            axios.post("https://localhost:7061/api/RolPermiso/Guardar", data)
          ));
          console.log('Relaciones rol-permiso guardadas.');
  
          // Muestra el modal de agradecimiento
          this.showThankYouModal();
          this.mostrarAlerta(); // Asegúrate de que esta función exista
          this.$router.push('RolTable'); // Ajusta la ruta de redirección según sea necesario
        } catch (error) {
          console.error('Error al guardar:', error.response ? error.response.data : error.message); // Más información sobre el error
          this.$alert('Ocurrió un error al guardar el Perfil.');
        } finally {
          this.hideModal();
        }
      }
