            ...
            <div class="form-container">
              <form @submit.prevent="showModal">
                <div class="input-group">
                    ...
                    <input type="text" id="nombreCompleto" name="nombreCompleto" placeholder="Nombre Completo" required v-model="form.nombre" readonly>
...
export default {
  ...
  methods: {
    flecha(){
        this.$router.push('/PermisoTable'); // Ajusta la ruta de redirección según sea necesario
      },
    ...
    async cargarDatosUsuario() {
      const idPermiso = this.$route.params.idPermiso; // Suponiendo que pasas el ID del usuario por la URL
      try {
        const response = await axios.get(`https://localhost:7061/api/Permiso/Obtener/${idPermiso}`);
    ...
    async submitForm() {
      try {
        ...
        await axios.put(`https://localhost:7061/api/Permiso/Editar`, this.form);
        
        this.$alert('Permiso actualizado correctamente!');
        this.$router.push('PermisoTable');
      } catch (error) {
        console.error('Error al actualizar el Permiso:', error);
        this.$alert('Ocurrió un error al actualizar el Permiso.');
      } finally {
        this.hideModal();
      }
    }
